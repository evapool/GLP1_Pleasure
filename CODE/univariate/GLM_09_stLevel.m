...
name_ana = 'GLM-09_1'; % output folder for this analysis
...
for i = 1%2:length(subj)
    
%     %i = i +41 ;
%     
%     if strcmp(sessionX, "second")
%         if i ==  8 || i == 12 || i == 35 || i == 43 %fails at 208, 213, 239, 249
%             continue
%         end
%     else
%          if i ==  42 %268 fails
...
%                         SPM.Sess(ses).C.name = rnam;
    ...
    function [] = doContrasts(subjoutdir, param, SPM)
        ...
        % con1
        Ctnames{1} = 'taste';
        weightPos  = ismember(conditionName, {'task1.taste.conc'}) * 1;
        Ct(1,:)    = weightPos;
        
        % con2
        Ctnames{2} = 'tasteXlik';
        weightPos  = ismember(conditionName, {'task1.taste.concxlik^1'}) * 1;
        Ct(2,:)    = weightPos;
...
%         
% %         % con2
% %         Ctnames{6} = 'rewardXlik-controlXlik';
% %         weightPos  = ismember(conditionName, {'task1.taste.rewardxlik^1'}) * 1;
% %         weightNeg  = ismember(conditionName, {'task1.taste.controlxlik^1'}) * -1;
